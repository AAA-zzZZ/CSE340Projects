...
public:
    ...
    DFA NFA2DFA(); 
    static NFA PostfixToNFA(const string& postFix, const set<char> alphabet);
    //new function moving from an NFA state to next state based on input symbol c
    set<int> Move(set<int> current_states, char c);
    void Print() const;
    int MaxStateId() const;


    //helper functions for thompsons construction
    void OffsetStates(int start_state);
    void AddEmptyTransition(int from, int to);
    void CombineTransitions(const NFA& other);
    int SingleAcceptState() const;
